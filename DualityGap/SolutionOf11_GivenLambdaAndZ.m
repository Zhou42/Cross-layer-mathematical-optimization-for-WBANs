function [ t_tilde, P_tilde, T_tilde, optimalValue ] = SolutionOf11_GivenLambdaAndZ( lambda, relay_idx )
%SOLUTIONOF11_GIVENLAMBDAANDZ Summary of this function goes here
...
cvx_begin quiet
    ...
    maximize(t_tilde - lambda * (sum(exp(T_tilde(1:S_num))) + sum(exp(T_tilde(relay_idx))) - T_frame));
%     maximize(t_tilde);
    subject to
...
optimalValue = t_tilde - lambda * (sum(exp(T_tilde(1:S_num))) + sum(exp(T_tilde(relay_idx))) - T_frame);
