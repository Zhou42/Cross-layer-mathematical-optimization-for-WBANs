function [ t_tilde, P_tilde, T_tilde ] = primalOptimalGivenZandSensorIdx( relay_idx, sensor_flag )
...
%         T_tilde(1:3) + log(W * log_sci(1 + (alpha_inBody(1:3,19).*exp(P_tilde(1:3)))/N(19))) >= log(x_s(1:3))
        ...
        % Region 1
        log(r_relay(relay_idx(1)) * exp(T_tilde(relay_idx(1)))) >= log(sum(x_s(1:3) .* sensor_flag(1:3) * T_frame))
        % Region 2
        log(r_relay(relay_idx(2)) * exp(T_tilde(relay_idx(2)))) >= log(sum(x_s(4:6) .* sensor_flag(4:6)* T_frame))
        % Region 3
        log(r_relay(relay_idx(3)) * exp(T_tilde(relay_idx(3)))) >= log(sum(x_s(7:9) .* sensor_flag(7:9) * T_frame))
        % Region 4
        log(r_relay(relay_idx(4)) * exp(T_tilde(relay_idx(4)))) >= log(sum(x_s(10:12) .* sensor_flag(10:12) * T_frame))
        % Region 5
        log(r_relay(relay_idx(5)) * exp(T_tilde(relay_idx(5)))) >= log(sum(x_s(13) .* sensor_flag(13)* T_frame))
        % Region 6
        log(r_relay(relay_idx(6)) * exp(T_tilde(relay_idx(6)))) >= log(sum(x_s(14:17) .* sensor_flag(14:17) * T_frame))
