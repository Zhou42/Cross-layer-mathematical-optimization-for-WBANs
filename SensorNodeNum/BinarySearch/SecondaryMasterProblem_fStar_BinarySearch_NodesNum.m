function [ t_tilde, z, T_tilde, P_tilde] = SecondaryMasterProblem_fStar_BinarySearch_NodesNum(lambda, sensor_flag)
...
[P_tilde, T_tilde, relay_idx, NO_SOLUTION_FLAG] = SecondaryMaster_SubProblemsAll_NodesNum(lambda,t_tilde, sensor_flag);
...
while abs(t_tilde_up - t_tilde_low) > threshold * F
    ...
    [P_tilde, T_tilde, relay_idx, NO_SOLUTION_FLAG] = SecondaryMaster_SubProblemsAll_NodesNum(lambda,t_tilde_mid, sensor_flag);
    
    if NO_SOLUTION_FLAG ~= true
        % if solution exist; calculate "t_tilde_delta"
        z = zeros(S_num + R_num, 1);
        z(relay_idx) = 1;
        gamma = getOptimalGamma_KKT_NodesNum( lambda, t_tilde_mid, T_tilde, P_tilde, z, relay_idx, sensor_flag );
        % here gamma only contains the sensors that are chosen!
