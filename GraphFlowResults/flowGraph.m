...
load('../Results_40kbps_400ms_binarysearch_decomposition.mat');
...
r(13) = W * log_sci((alpha_inBody(13,relay_idx(5)).*exp(P_tilde(13)))/N(relay_idx(5)));
r(14:17) = W * log_sci((alpha_inBody(14:17,relay_idx(6)).*exp(P_tilde(14:17)))/N(relay_idx(6)));
% data of relays
r_relay(relay_idx);
...
for i = 1:S_num
    hold on;
    plot([X(i,1) X(map(i, 2),1)], [X(i,2) X(map(i, 2),2)], 'Linewidth',r(i) * 15/1e7,'color','k');
end

%% plot relay to coordinator
for i = 1:numel(relay_idx)
    hold on;
    plot([X(relay_idx(i),1) X(S_num + R_num + 1,1)], [X(relay_idx(i),2) X(S_num + R_num + 1,2)], 'Linewidth',r(i) * 15/1e7,'color','k');
    r_relay
end
