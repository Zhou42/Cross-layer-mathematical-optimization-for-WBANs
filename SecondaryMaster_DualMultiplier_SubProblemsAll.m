...
%         T_tilde(1:3) + log(W * log_sci(1 + (alpha_inBody(1:3,19).*exp(P_tilde(1:3)))/N(19))) >= log(x_s(1:3))
        % Region 1
        T_tilde_temp(1:3) + log(W * log_sci((alpha_inBody(1:3,relay_idx(1)).*exp(P_tilde_temp(1:3)))/N(relay_idx(1)))) >= log(x_s(1:3))
        % Region 2
        T_tilde_temp(4:6) + log(W * log_sci((alpha_inBody(4:6,relay_idx(2)).*exp(P_tilde_temp(4:6)))/N(relay_idx(2)))) >= log(x_s(4:6))
        % Region 3
        T_tilde_temp(7:9) + log(W * log_sci((alpha_inBody(7:9,relay_idx(3)).*exp(P_tilde_temp(7:9)))/N(relay_idx(3)))) >= log(x_s(7:9))
        % Region 4
        T_tilde_temp(10:12) + log(W * log_sci((alpha_inBody(10:12,relay_idx(4)).*exp(P_tilde_temp(10:12)))/N(relay_idx(4)))) >= log(x_s(10:12))
        % Region 5
        T_tilde_temp(13:17) + log(W * log_sci((alpha_inBody(13:17,relay_idx(5)).*exp(P_tilde_temp(13:17)))/N(relay_idx(5)))) >= log(x_s(13:17))
        
        % Region 1
        log(r_relay(relay_idx(1)) * exp(T_tilde_temp(relay_idx(1)))) >= log(sum(x_s(1:3)))
        % Region 2
        log(r_relay(relay_idx(2)) * exp(T_tilde_temp(relay_idx(2)))) >= log(sum(x_s(4:6)))
        % Region 3
        log(r_relay(relay_idx(3)) * exp(T_tilde_temp(relay_idx(3)))) >= log(sum(x_s(7:9)))
        % Region 4
        log(r_relay(relay_idx(4)) * exp(T_tilde_temp(relay_idx(4)))) >= log(sum(x_s(10:12)))
        % Region 5
        log(r_relay(relay_idx(5)) * exp(T_tilde_temp(relay_idx(5)))) >= log(sum(x_s(13:17)))
